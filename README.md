...
本仓库即为作业提交入口（BOSS 直聘仅支持发送仓库链接、无法发附件，故全部材料随仓库提供）：

| 提交物 | 仓库内路径 | 对应要求 |
|---|---|---|
| 代码（真实 LLM API） | 本仓库全部源码 | ① 真实 API：key 存 `.env`（已 gitignore），`.env.example` 为模板；② 代码链接 |
| `README.md` | 仓库根 | ③ 运行方式 / 系统设计 / Memory 召回时机与放置 / 目录结构 |
| `AI_LOG.md` | `提交物/AI_LOG.md` | ④ AI Prompt 与问题解决记录（已脱敏：无公司名 / 笔试题 / 面试官） |
| 架构设计作答 | `提交物/架构设计-滑动窗口限流四问作答.md` | 滑动窗口限流四问：数据结构与核心逻辑 / 复杂度 / 四种算法场景 / Redis 集群限流设计 |
| 演示录屏 | `提交物/视频演示.mp4` | 3~5 分钟功能演示（自研 Runtime / 四步循环 / 工具可视化 / 多 Session / 上下文记忆 / Trace） |

> 说明：`.env` 与 `doc/`（内部过程文档）按规则不纳入公开仓库；`AI_LOG.md` 与录屏此前为单独交付件，现因平台限制统一随仓库提交于 `提交物/` 目录。
